...
import type { SshLocalForwardAction } from "../../lib/terminal/sshLocalForwardTypes";
import {
...
import { Cable, Loader2, X } from "../icons";
...
export function SshPortForwardDialog(props: SshPortForwardDialogProps) {
  const { sessionId, projectPathKey, subtitle, onClose, onStarted } = props;
  const { t } = useLocale();
    ...
    void (async () => {
      try {
        if (target.localPort > 0) {
          ...
          const available = await tauriSshLocalForwardClient
            .checkLocalPort(target.localPort)
            .catch(() => true);
          if (!available) {
        ...
        const action = await tauriSshLocalForwardClient.start({
          sessionId,
          projectPathKey,
