...
const DELEGATE_EXECUTION_MODE_SCHEMA = Type.Union(
  ...
  {
    description:
      "readonly for message/research/review agents; worktree for isolated file changes.",
  },
);
...
export const DELEGATE_AGENT_PARAMETERS = Type.Object({
  ...
  id: Type.Optional(
    Type.String({
      description:
        "Stable id for one agent. Reuse the same id to resume that agent.",
    }),
  ),
  name: Type.Optional(
    Type.String({
  ...
  agent_id: Type.Optional(
    Type.String({
      description:
        "Optional configured AGENTS template id or name for one agent.",
    }),
  ),
  ...
  allowed_output_paths: Type.Optional(
    Type.Array(Type.String(), {
      description:
        "Relative files/directories allowed when apply_policy=explicit.",
    }),
  ),
