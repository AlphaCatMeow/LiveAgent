...
| WebUI | `crates/agent-gateway/web` | React、TypeScript、Vite、WebSocket、SharedWorker | 远程浏览器端 Chat/Settings/Hub 壳层，通过 Gateway 操作本地 Agent。 |
...
| WebUI 远程对话 | WebUI 发 `chat.start` 到 Gateway，Gateway 通过 gRPC `AgentConnect` 下发 `ChatRequest` 到桌面端，桌面端本地运行并持续回传 `ChatEvent`，Gateway 按 seq 广播给 WebUI。 | `web/src/lib/gatewaySocket.ts`、`internal/server/websocket.go`、`proto/v1/gateway.proto`、`src-tauri/src/services/gateway_bridge.rs` |
...
| Gateway 不越权 | Gateway 不直接访问用户文件系统，不保存真实 provider key，只维护会话、中继和可恢复 buffer。 |
| GUI/WebUI 可用性对齐 | WebUI 复制/镜像了大量 GUI 组件与 settings 子树，但用 shims 和 Gateway client 替换 Tauri API。 |
| 长对话可恢复 | 历史使用 segment + summary checkpoint，Gateway chat run 使用 seq buffer，WebUI 可 resume/attach。 |
...
Browser WebUI
  ...
  ├─ GatewayWebSocketClient or SharedWorker
...
Go Gateway
  ...
  └─ session.Manager: agent session, streams, settings/history subscribers, chat run buffer
        │
        ▼
Desktop LiveAgent
