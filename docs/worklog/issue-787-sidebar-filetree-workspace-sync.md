...
- Desktop `ChatPage.handleSelectConversation` 与 WebUI `handleSidebarSelectConversation` 的普通点选分支，在 `openController.open` 之前调用它。cwd 取侧栏行的持久化 cwd，取不到再退到运行时缓存的 workdir。只在 agent 模式下做，text 模式不动工作空间。

不改 `activateWorkspaceProject` 本身，不动搜索路径与 Workbench 路径的语义。
